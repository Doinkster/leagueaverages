...
class ChampionSelect extends Component {
  constructor(props) {
    ...
    this.sendSelectedChampsToSummonerParent = this.sendSelectedChampsToSummonerParent.bind(this)
  }
  ...
  render() {
    ...
    const championList = this.state.champions.map(champName => {
      ...
      nameWithoutPuncuation = nameWithoutPuncuation.toLowerCase().split(' ').map(function(x) {return x[0].toUpperCase() + x.substr(1)}).join(' ')
      ...
      if(this.props.championsPlayed[championIds[nameWithoutPuncuation]]) {
        return (
          <div key={champName} className={champName} onClick={this.sendSelectedChampsToSummonerParent}>
            <button className={champName} style={{backgroundColor: buttonColor}} />
